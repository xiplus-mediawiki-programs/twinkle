...
async function main() {
    ...
    try {
        bot = await Mwn.init({
            apiUrl: apiUrl,
            userAgent: 'deploy.js (https://github.com/xiplus-mediawiki-programs/twinkle) (mwn)',
    ...
    for (const file of files) {
        ...
        // Hamish's user scripts has different naming scheme, the same below
        if (GADGET_FILES.indexOf(file) < 3 && conf.base === DEFAULT_CONF.base) {
            page = page.charAt(0).toUpperCase() + page.slice(1);
        }

        
        if (!page.startsWith(conf.base) ) {
            if (conf.base === DEFAULT_CONF.base) {
