...
Twinkle.fluff = {
	auto: function() {
		if( parseInt( Morebits.queryString.get('oldid'), 10) !== mw.config.get('wgCurRevisionId') ) {
			// not latest revision
			alert(wgUVS("无法回退，页面在此期间已被修改。", "無法回退，頁面在此期間已被修改。"));
			return;
		}
		...
		if( mw.config.get('wgNamespaceNumber') === -1 && mw.config.get('wgCanonicalSpecialPageName') === "Contributions" ) {
			...
			if (logLink.length>0) //#215 -- there is no log link on Special:Contributions with no user
			{
				...
				if( Twinkle.getPref('showRollbackLinks').indexOf('contribs') !== -1 ||
					...
					revVandLink.appendChild( spanTag( 'Red', wgUVS('破坏', '破壞') ) );
					revVandLink.appendChild( spanTag( 'Black', ']' ) );
			...
			revAgfLink.appendChild( spanTag( 'DarkOliveGreen', wgUVS('回退（AGF）', '回退（AGF）') ) );
			revAgfLink.appendChild( spanTag( 'Black', ']' ) );
			...
			revertToRevisionLink.appendChild( spanTag( 'SaddleBrown', wgUVS('恢复此版本', '恢復此版本') ) );
			revertToRevisionLink.appendChild( spanTag( 'Black', ']' ) );
			...
			if( Twinkle.getPref('showRollbackLinks').indexOf('diff') !== -1 ) {
				...
				vandLink.appendChild( spanTag( 'Red', wgUVS('回退（破坏）', '回退（破壞）') ) );
				vandLink.appendChild( spanTag( 'Black', ']' ) );
...
Twinkle.fluff.revert = function revertPage( type, vandal, autoRevert, rev, page ) {
	...
	if (Twinkle.fluff.useNotify) {
		...
		statusElement.textContent = wgUVS('正在准备回退……', '正在準備回退……');
		if (Twinkle.fluff.isRTRC) {
			// TODO 实现反馈
	...
	var wikipedia_api = new Morebits.wiki.api( wgUVS('抓取较早修订版本信息', '擷取較早修訂版本資訊'), query, Twinkle.fluff.callbacks.main );
...
Twinkle.fluff.revertToRevision = function revertToRevision( oldrev, page, summary ) {
	...
	var wikipedia_api = new Morebits.wiki.api( wgUVS('抓取较早修订版本信息', '擷取較早修訂版本資訊'), query, Twinkle.fluff.callbacks.toRevision.main );
...
Twinkle.fluff.callbacks = {
	toRevision: {
		main: function( self ) {
			...
			if (revertToRevID !== self.params.rev) {
				self.statelem.error( wgUVS('抓取到的修订版本与请求的修订版本不符，取消。', '擷取到的修訂版本與請求的修訂版本不符，取消。') );
				return;
			}

			var optional_summary = prompt( wgUVS("请输入回退理由：", "請輸入回退理由：")+"                                ", self.params.summary );  // padded out to widen prompt in Firefox
			if (optional_summary === null)
			{
				self.statelem.error( wgUVS('由用户取消。', '由用戶取消。') );
				return;
			}
			var summary = Twinkle.fluff.formatSummary(wgUVS("回退到由$USER做出的修订版本", "回退到由$USER做出的修訂版本") + revertToRevID, revertToUser, optional_summary);
			...
			Morebits.wiki.actionCompleted.notice = wgUVS("修订版本完成", "修訂版本完成");

			var wikipedia_api = new Morebits.wiki.api( wgUVS('保存回退内容', '儲存回退內容'), query, Twinkle.fluff.callbacks.complete, self.statelem);
	...
	main: function( self ) {
		...
		if( revs.length < 1 ) {
			self.statelem.error( wgUVS('没有其它修订版本，无法回退', '沒有其它修訂版本，無法回退') );
			return;
		}
		var top = revs[0];
		if( lastrevid < self.params.revid ) {
			Morebits.status.error( wgUVS('错误', '錯誤'), wgUVS([ '从服务器获取的最新修订版本ID ', Morebits.htmlNode( 'strong', lastrevid ), ' 小于目前所显示的修订版本ID。这可能意味着当前修订版本已被删除、服务器延迟、或抓取到了坏掉的信息。取消。' ], [ '從伺服器取得的最新修訂版本ID ', Morebits.htmlNode( 'strong', lastrevid ), ' 小於目前所顯示的修訂版本ID。這可能意味著當前修訂版本已被刪除、伺服器延遲、或擷取到了壞掉的資訊。取消。' ]) );
			return;
		}
		var index = 1;
		if( self.params.revid !== lastrevid  ) {
			Morebits.status.warn( '警告', wgUVS([ '最新修订版本 ', Morebits.htmlNode( 'strong', lastrevid ), ' 与我们的修订版本 ', Morebits.htmlNode( 'strong', self.params.revid ), '不等' ], [ '最新修訂版本 ', Morebits.htmlNode( 'strong', lastrevid ), ' 與我們的修訂版本 ', Morebits.htmlNode( 'strong', self.params.revid ), ' 不等' ]) );
			if( lastuser === self.params.user ) {
				switch( self.params.type ) {
				case 'vand':
					Morebits.status.info( '信息', wgUVS([ '最新修订版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，因我们假定破坏，继续回退操作。' ], [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，因我們假定破壞，繼續回退操作。' ]));
					break;
				case 'agf':
					Morebits.status.warn( '警告', wgUVS([ '最新修订版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，因我们假定善意，取消回退操作，因为问题可能已被修复。' ], [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，因我們假定善意，取消回退操作，因為問題可能已被修復。' ]));
					return;
				default:
					Morebits.status.warn( '提示', wgUVS([ '最新修订版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，但我们还是不回退了。' ], [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，但我們還是不回退了。' ]) );
					return;
				}
			}
				...
				Morebits.status.info( '信息', wgUVS([ '最新修订版本由 ', Morebits.htmlNode( 'strong', lastuser ), '，一个可信的机器人做出，之前的版本被认为是破坏，继续回退操作。' ], [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', lastuser ), '，一個可信的機器人做出，之前的版本被認為是破壞，繼續回退操作。' ]) );
				index = 2;
			} else {
				Morebits.status.error( '错误', wgUVS([ '最新修订版本由 ', Morebits.htmlNode( 'strong', lastuser ), ' 做出，所以这个修订版本可能已经被回退了，取消回退操作。'], [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', lastuser ), ' 做出，所以這個修訂版本可能已經被回退了，取消回退操作。']) );
				return;
			}
		...
		if( Twinkle.fluff.whiteList.indexOf( self.params.user ) !== -1  ) {
			switch( self.params.type ) {
			case 'vand':
				Morebits.status.info( '信息', wgUVS([ '将对 ', Morebits.htmlNode( 'strong', self.params.user ), ' 执行破坏回退，这是一个可信的机器人，我们假定您要回退前一个修订版本。' ], [ '將對 ', Morebits.htmlNode( 'strong', self.params.user ), ' 執行破壞回退，這是一個可信的機器人，我們假定您要回退前一個修訂版本。' ]) );
				index = 2;
				self.params.user = revs[1].getAttribute( 'user' );
				break;
			case 'agf':
				Morebits.status.warn( '提示', wgUVS([ '将对 ', Morebits.htmlNode( 'strong', self.params.user ), ' 执行善意回退，这是一个可信的机器人，取消回退操作。' ], [ '將對 ', Morebits.htmlNode( 'strong', self.params.user ), ' 執行善意回退，這是一個可信的機器人，取消回退操作。' ]) );
				return;
			case 'norm':
				/* falls through */
			default:
				var cont = confirm( wgUVS('选择了常规回退，但最新修改是由一个可信的机器人（' + self.params.user + '）做出的。您是否想回退前一个修订版本？', '選擇了常規回退，但最新修改是由一個可信的機器人（' + self.params.user + '）做出的。您是否想回退前一個修訂版本？') );
				if( cont ) {
					Morebits.status.info( '信息', wgUVS([ '将对 ', Morebits.htmlNode( 'strong', self.params.user ), ' 执行常规回退，这是一个可信的机器人，基于确认，我们将回退前一个修订版本。' ], [ '將對 ', Morebits.htmlNode( 'strong', self.params.user ), ' 執行常規回退，這是一個可信的機器人，基於確認，我們將回退前一個修訂版本。' ]) );
					index = 2;
					self.params.user = revs[1].getAttribute( 'user' );
				} else {
					Morebits.status.warn( '提示', wgUVS([ '将对 ', Morebits.htmlNode( 'strong', self.params.user ), ' 执行常规回退，这是一个可信的机器人，基于确认，我们仍将回退这个修订版本。' ], [ '將對 ', Morebits.htmlNode( 'strong', self.params.user ), ' 執行常規回退，這是一個可信的機器人，基於確認，我們仍將回退這個修訂版本。' ]) );
				}
				break;
			}
		}
		...
		if( ! found ) {
			self.statelem.error( wgUVS([ '未找到之前的修订版本，可能 ', Morebits.htmlNode( 'strong', self.params.user ), ' 是唯一贡献者，或这个用户连续做出了超过 ' + Twinkle.getPref('revertMaxRevisions') + ' 次编辑。' ], [ '未找到之前的修訂版本，可能 ', Morebits.htmlNode( 'strong', self.params.user ), ' 是唯一貢獻者，或這個用戶連續做出了超過 ' + Twinkle.getPref('revertMaxRevisions') + ' 次編輯。' ]) );
			return;
		}

		if( ! count ) {
			Morebits.status.error( wgUVS('错误', '錯誤'), wgUVS("我们将要回退0个修订版本，这没有意义，所以取消回退操作。可能是因为这个修订版本已经被回退，但修订版本ID仍是一样的。", "我們將要回退0個修訂版本，這沒有意義，所以取消回退操作。可能是因為這個修訂版本已經被回退，但修訂版本ID仍是一樣的。") );
			return;
		}
		...
		if (self.params.type !== 'vand' && count > 1) {
			if ( !confirm( wgUVS(self.params.user + ' 连续做出了 ' + count + ' 次编辑，是否要回退所有这些？', self.params.user + ' 連續做出了 ' + count + ' 次編輯，是否要回退所有這些？')) ) {
				Morebits.status.info( '提示', wgUVS('用户取消操作', '用戶取消操作') );
				return;
			}
		...
		self.statelem.status( [ Morebits.htmlNode( 'strong', count ), wgUVS(' 个修订版本之前由 ', ' 個修訂版本之前由 '), Morebits.htmlNode( 'strong', self.params.gooduser ), wgUVS(' 做出的修订版本 ', ' 做出的修訂版本 '), Morebits.htmlNode( 'strong', self.params.goodid ) ] );

		var summary, extra_summary;
		switch( self.params.type ) {
		case 'agf':
			extra_summary = prompt( wgUVS("可选的编辑摘要：", "可選的編輯摘要：")+"                              ", self.params.summary );  // padded out to widen prompt in Firefox
			if (extra_summary === null)
			{
				self.statelem.error( wgUVS('用户取消操作。', '用戶取消操作。') );
				return;
			}
			userHasAlreadyConfirmedAction = true;

			summary = Twinkle.fluff.formatSummary(wgUVS("回退$USER做出的出于[[WP:AGF|善意]]的编辑", "回退$USER做出的出於[[WP:AGF|善意]]的編輯"), self.params.user, extra_summary);
			break;
		...
		case 'vand':
			...
			summary = "回退[[Special:Contributions/" +
				...
				"做出的 " + self.params.count + wgUVS(" 次编辑，到由", " 次編輯，到由") +
				self.params.gooduser + wgUVS("做出的前一个修订版本 ", "做出的前一個修訂版本 ")  + Twinkle.getPref('summaryAd');
			break;
		...
		default:
			...
			summary = Twinkle.fluff.formatSummary(wgUVS("回退$USER做出的" + self.params.count + "次编辑", "回退$USER做出的" + self.params.count + "次編輯"), self.params.user, extra_summary);
			break;
		}

		if (Twinkle.getPref('confirmOnFluff') && !userHasAlreadyConfirmedAction && !confirm(wgUVS("回退页面：您确定吗？", "回退頁面：您確定嗎？"))) {
		...
		if( (!self.params.autoRevert || Twinkle.getPref('openTalkPageOnAutoRevert')) &&
			...
			Morebits.status.info( '信息', wgUVS([ '打开用户 ', Morebits.htmlNode( 'strong', self.params.user ), ' 的对话页' ], [ '開啟用戶 ', Morebits.htmlNode( 'strong', self.params.user ), ' 的對話頁' ]) );
	...
	complete: function (apiobj) {
		...
		if (blacklist) {
			...
			apiobj.statelem.error(wgUVS(['不能回退，因URL', code, '在垃圾黑名单中。'], ['不能回退，因URL', code, '在垃圾黑名單中。']));
		} else if ( $(xml).find('captcha').length > 0 ) {
			apiobj.statelem.error(wgUVS("不能回退，因维基服务器要求您输入验证码。", "不能回退，因維基伺服器要求您輸入驗證碼。"));
		} else if ( $edit.attr('code') === 'abusefilter-disallowed' ) {
			apiobj.statelem.error(wgUVS('此编辑被滥用过滤器“' + $edit.attr('info').substring(17) + '”禁止。', '此編輯被濫用過濾器「' + $edit.attr('info').substring(17) + '」禁止。'));
			...
			apiobj.statelem.error(wgUVS([ '编辑过滤器返回了以下警告：', div, '如果您仍希望回退，请重新整理本页（F5或Ctrl+R）然后重试，此警告将不会再次出现。' ], [ '編輯過濾器返回了以下警告：', div, '如果您仍希望回退，請重新整理本頁（F5或Ctrl+R）然後重試，此警告將不會再次出現。' ]));
		} else if ($edit.attr('nochange') === '') {
			apiobj.statelem.warn(wgUVS("要回退到的版本与当前版本相同，没什么要做的", "要回退到的版本與當前版本相同，沒什麼要做的"));
		} else {
...
Twinkle.fluff.formatSummary = function(builtInString, userName, userString) {
	...
	if (resultLen + contribsLen <= 255) {
		var talkLink = " ([[User talk:" + userName + wgUVS("|讨论]])", "|討論]])");
