	...
	switch (value) {
		...
		case 'autolevel':
			...
			if (Twinkle.warn.talkpageObj) {
				...
				var usertalk_page = new Morebits.wiki.Page('User_talk:' + mw.config.get('wgRelevantUserName'), conv({ hans: '加载上次警告', hant: '載入上次警告' }));
...
Twinkle.warn.callbacks = {
	...
	showPreview: function(form, templatename) {
		...
		form.previewer.beginRender(templatetext, 'User_talk:' + mw.config.get('wgRelevantUserName'));
	},
			...
			if (autoTimeout.isAfter(date)) {
				if (level === 4) {
					...
					if (!statelem) {
						...
						var statusNode = $('<div/>', {
							text: mw.config.get('wgRelevantUserName') + conv({ hans: '最后收到了一个层级4警告（', hant: '最後收到了一個層級4警告（' }) + latest.type + conv({ hans: '），所以将其报告给管理人员会比较好；', hant: '），所以將其報告給管理人員會比較好；' }),
							css: {color: 'red' }
						});
