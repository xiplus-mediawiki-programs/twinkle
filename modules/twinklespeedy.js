...
Twinkle.speedy.articleList = [
	{
		...
		value: 'a1',
		tooltip: wgULS('条目的内容笼统，或甚至根本没有提及条目主体，使条目不能用以区分其他事物。例如：“他是一个很有趣的人，他创建了工厂和庄园。并且，顺便提一下，他的妻子也很好。”<br>“内容空泛”与“没有实际内容”的分别在于：后者乃是与主题完全无关，或是不知所云；前者可以与主题有一定关系，但没有明确描述主题（包括但不限于没有对主题作基本定义）；前者同时包含后者。', '條目的內容籠統，或甚至根本沒有提及條目主體，使條目不能用以區分其他事物。例如：「他是一個很有趣的人，他建立了工廠和莊園。並且，順便提一下，他的妻子也很好。」<br>「內容空泛」與「沒有實際內容」的分別在於：後者乃是與主題完全無關，或是不知所云；前者可以與主題有一定關係，但沒有明確描述主題（包括但不限於沒有對主題作基本定義）；前者同時包含後者。')
	},
	{
		label: wgULS('A2: 内容只包括外部链接、参见、图书参考、分类、模板、跨语言链接的条目', 'A2: 內容只包括外部連結、參見、圖書參考、分類、模板、跨語言連結的條目'),
		value: 'a2',
		tooltip: wgULS('请注意：有些维基人创建条目时会分开多次保存，请避免删除有人正在编辑的页面。<br>带有{{inuse}}模板的不适用。', '請注意：有些維基人建立條目時會分開多次儲存，請避免刪除有人正在編輯的頁面。<br>帶有{{inuse}}模板的不適用。')
	},
	{
	...
	},
	{
		...
		value: 'a5',
		tooltip: wgULS('条目被创建时，第一个版本的内容与当时其他现存条目或其历史版本的全部或部分内容完全相同或非常相似，且这个条目的名称不适合改为重定向，就可以提送快速删除。<br>如果名称可以作为重定向，就应直接改重定向，不要提送快速删除。<br>如果是多个条目合并产生的新条目，不适用。<br>如果是从主条目拆分产生的条目，不适用；如有疑虑，应提送存废讨论处理。', '條目被建立時，第一個版本的內容與當時其他現存條目或其歷史版本的全部或部分內容完全相同或非常相似，且這個條目的名稱不適合改為重新導向，就可以提送快速刪除。<br>如果名稱可以作為重新導向，就應直接改重新導向，不要提送快速刪除。<br>如果是多個條目合併產生的新條目，不適用。<br>如果是從主條目拆分產生的條目，不適用；如有疑慮，應提送存廢討論處理。'),
		subgroup: {
			name: 'a5_pagename',
			type: 'input',
			label: wgULS('现有条目名：', '現有條目名：'),
	...
	},
	{
		...
		value: 'a6',
		tooltip: wgULS('如果并不是复制于任何其他的维基百科语言版本，请换用{{notmandarin}}。<br>带有{{inuse}}和{{translating}}模板的不适用。', '如果並不是複製於任何其他的維基百科語言版本，請換用{{notmandarin}}。<br>帶有{{inuse}}和{{translating}}模板的不適用。'),
		subgroup: {
			name: 'a6_pagename',
			type: 'input',
			label: wgULS('现有条目名：', '現有條目名：'),
...
Twinkle.speedy.draftList = [
	{
		...
		value: 'o7',
		tooltip: wgULS('任何六个月内无编辑的草稿。', '任何六個月內無編輯的草稿。')
	}
];
...
Twinkle.speedy.userList = [
	{
		...
		value: 'o1',
		tooltip: wgULS('除了自己的用户页，只要是移动而来的页面，皆须附有合理原因。', '除了自己的使用者頁面，只要是移動而來的頁面，皆須附有合理原因。')
	}
];
...
Twinkle.speedy.usertalkList = [
	{
		...
		value: 'o3',
		tooltip: wgULS('避免给使用同一IP地址的用户带来混淆。<br>不适用于用户讨论页的存档页面。', '避免給使用同一IP位址的使用者帶來混淆。<br>不適用於使用者討論頁的存檔頁面。')
	}
];
...
Twinkle.speedy.generalList = [
	...
	},
	{
		...
		value: 'g8',
		tooltip: wgULS('包括解封用户后删除用户页、因用户夺取而删除、删除无用的MediaWiki页面、因移动请求而删除页面、以改写删除重定向。', '包括解封使用者後刪除使用者頁面、因使用者奪取而刪除、刪除無用的MediaWiki頁面、因移動請求而刪除頁面、以覆寫刪除重新導向。'),
		hideWhenUser: true
	},
	{
		label: wgULS('G10: 原作者清空页面或提出删除，且实际贡献者只有一人', 'G10: 原作者清空頁面或提出刪除，且實際貢獻者只有一人'),
		value: 'g10',
		tooltip: wgULS('提请须出于善意，并附有合理原因。', '提請須出於善意，並附有合理原因。'),
		subgroup: {
			name: 'g10_rationale',
			type: 'input',
	...
	},
	{
		...
		value: 'g11',
		tooltip: wgULS('页面只收宣传之用，并须完全重写才能贴合百科全书要求。须注意，仅仅以某公司或产品为主题的条目，并不直接导致其自然满足此速删标准。<br>即便该页面具有宣传情况，除非您可以非常确定该页面创建仅为广告宣传而建，否则应以关注度提报或提删替代。', '頁面只收宣傳之用，並須完全重寫才能貼合百科全書要求。須注意，僅僅以某公司或產品為主題的條目，並不直接導致其自然滿足此速刪標準。<br>即便該頁面具有宣傳情況，除非您可以非常確定該頁面建立僅為廣告宣傳而建，否則應以關注度提報或提刪替代。')
	},
	{
		label: wgULS('G12: 未列明可靠来源且语调负面的生者传记', 'G12: 未列明可靠來源且語調負面的生者傳記'),
		value: 'g12',
		tooltip: wgULS('“列明”指至少一个列出的来源可直接支撑条目中的任何信息，并与其中的断言相匹配。<br>如有用户对条目中列出的来源是否属于可靠来源提出合理异议，则应交由存废讨论处理。<br>注意是未列明可靠来源且语调负面，须2项均符合方适用此项。', '「列明」指至少一個列出的來源可直接支撐條目中的任何資訊，並與其中的斷言相匹配。<br>如有使用者對條目中列出的來源是否屬於可靠來源提出合理異議，則應交由存廢討論處理。<br>注意是未列明可靠來源且語調負面，須2項均符合方適用此項。')
	},
	{
	...
	},
	{
		...
		value: 'g14',
		tooltip: wgULS('包括所有未翻译的外语、汉语方言以及文言文。<br>此项仅适用于条目、项目、维基专题、使用说明和主题命名空间。', '包括所有未翻譯的外語、漢語方言以及文言文。<br>此項僅適用於條目、計畫、維基專題、使用說明和主題命名空間。'),
		hideWhenUser: true,
	...
	},
	{
		...
		value: 'g15',
		tooltip: wgULS('包括以下几种类型：<br>1. 没有对应文件的文件页面；<br>2. 没有对应母页面的子页面，用户页子页面除外；<br>3. 指向不存在页面的重定向；<br>4. 没有对应内容页面的讨论页，讨论页存档和用户讨论页除外；<br>5. 不存在注册用户的用户页及用户页子页面，localhost对应IP用户的用户页和随用户更名产生的用户页重定向除外。<br>请在删除时注意有无将内容移至他处的必要。<br>不包括在主页面挂有{{CSD Placeholder}}模板的讨论页。', '包括以下幾種類型：<br>1. 沒有對應檔案的檔案頁面；<br>2. 沒有對應母頁面的子頁面，使用者頁面子頁面除外；<br>3. 指向不存在頁面的重新導向；<br>4. 沒有對應內容頁面的討論頁，討論頁存檔和使用者討論頁除外；<br>5. 不存在註冊使用者的使用者頁面及使用者頁面子頁面，localhost對應IP使用者的使用者頁面和隨使用者更名產生的使用者頁面重新導向除外。<br>請在刪除時注意有無將內容移至他處的必要。<br>不包括在主頁面掛有{{CSD Placeholder}}模板的討論頁。')
	},
	{
		label: wgULS('G17: 位于不恰当的命名空间的消歧义页面', 'G17: 位於不恰當的命名空間的消歧義頁面'),
		value: 'g17',
		tooltip: wgULS('在提请快速删除前，请务必先检查并清理相关消歧义页面的链入。<br>此项不论页面是否引用消歧义模板或消歧义消息模板均适用，惟对消歧义模板及消歧义消息模板本身不适用。', '在提請快速刪除前，請務必先檢查並清理相關消歧義頁面的連入。<br>此項不論頁面是否引用消歧義模板或消歧義訊息模板均適用，惟對消歧義模板及消歧義訊息模板本身不適用。'),
...
Twinkle.speedy.redirectList = [
	{
		...
		value: 'r2',
		tooltip: wgULS('适用于条目命名空间和草稿命名空间。<br>社群同意设立的伪命名空间不适用。<br>草稿重定向速删前，请确保草稿已经完成其作用，且其历史已移动到相应的正式页面。', '適用於條目命名空間和草稿命名空間。<br>社群同意設立的偽命名空間不適用。<br>草稿重新導向速刪前，請確保草稿已經完成其作用，且其歷史已移動到相應的正式頁面。'),
		showInNamespaces: [ 0, 118 ] // main, draft
	},
	{
		label: wgULS('R3: 格式错误，或明显笔误的重定向。', 'R3: 格式錯誤，或明顯筆誤的重新導向。'),
		value: 'r3',
		tooltip: wgULS('非一眼能看出的拼写错误和翻译或标题用字的争议应交由存废讨论处理。<br>将常见的拼写错误名称重定向至正确名称页面，可使百科用户纵使在查找文章时拼写错误，也能够找到寻求的文章。可参阅WP:重定向#何时用重定向？。<br>如重定向名称与导向目标名称（或其相关名称）仅存在合理的大小写差异及／或重定向名称为导向目标名称（或其相关名称）的ASCII字母表述的形式（例如Kurt Godel重定向至Kurt Gödel），不视为存在任何拼写错误。如有对相关大小写差异是否合理的争议，应交由存废讨论处理。<br>因类推简化字未收录至《通用规范汉字表》导致的繁简混杂情形，或系统无法自动进行繁简处理的情形，则不适用。', '非一眼能看出的拼寫錯誤和翻譯或標題用字的爭議應交由存廢討論處理。<br>將常見的拼寫錯誤名稱重新導向至正確名稱頁面，可使百科使用者縱使在尋找文章時拼寫錯誤，也能夠找到尋求的文章。可參閱WP:重新導向#何時用重新導向？。<br>如重新導向名稱與導向目標名稱（或其相關名稱）僅存在合理的大小寫差異及／或重新導向名稱為導向目標名稱（或其相關名稱）的ASCII字母表述的形式（例如Kurt Godel重新導向至Kurt Gödel），不視為存在任何拼寫錯誤。如有對相關大小寫差異是否合理的爭議，應交由存廢討論處理。<br>因類推簡化字未收錄至《通用規範漢字表》導致的繁簡混雜情形，或系統無法自動進行繁簡處理的情形，則不適用。'),
		subgroup: {
			name: 'r3_type',
			type: 'select',
	...
	},
	{
		...
		value: 'r7',
		tooltip: wgULS('条目完全未提及重定向的名称，或重定向名称比条目主题更广泛，但条目不含有重定向名称的有效介绍。同时，重定向名称并不是条目的别名或错误拼写。有争议的情况下，应提出存废讨论。<br>如果原重定向标题可改成消歧义页（或重定向至其他消歧义页），则不适用。<br>挂有{{关注度重定向}}或{{合并重定向}}模板的页面不适用，请改为提出存废讨论。<br>如有用户对标题用字存在未解决的争议，则应交由存废讨论处理。', '條目完全未提及重新導向的名稱，或重新導向名稱比條目主題更廣泛，但條目不含有重新導向名稱的有效介紹。同時，重新導向名稱並不是條目的別名或錯誤拼寫。有爭議的情況下，應提出存廢討論。<br>如果原重新導向標題可改成消歧義頁（或重新導向至其他消歧義頁），則不適用。<br>掛有{{關注度重新導向}}或{{合併重新導向}}模板的頁面不適用，請改為提出存廢討論。<br>如有使用者對標題用字存在未解決的爭議，則應交由存廢討論處理。')
	},
	{
		label: wgULS('R8: 带有“(消歧义)”字样，且无链入的重定向。', 'R8: 帶有「(消歧義)」字樣，且無連入的重新導向。'),
		value: 'r8',
		tooltip: wgULS('若重定向页与导向目标页同样带有“(消歧义)”字样，且两者的标题仅存在繁简／地区词差异，则不适用。<br在提请快速删除前，请务必先检查并清理（如适用）相关重定向的链入。<br>如有用户对应否使用消歧义及消歧义的方式存在未解决的争议，则应交由存废讨论处理。', '若重新導向頁面與導向目標頁同樣帶有「(消歧義)」字樣，且兩者的標題僅存在繁簡／地區詞差異，則不適用。<br在提請快速刪除前，請務必先檢查並清理（如適用）相關重新導向的連入。<br>如有使用者對應否使用消歧義及消歧義的方式存在未解決的爭議，則應交由存廢討論處理。')
	}
];
