...
Twinkle.speedy.redirectList = [
	...
	{
		label: conv({ hans: 'R7: 导向目标明显未介绍名称所指事物，或名称涵盖的主题明显比导向目标更广泛的重定向', hant: 'R7: 導向目標明顯未介紹名稱所指事物，或名稱涵蓋的主題明顯比導向目標更廣泛的重新導向。' }),
		value: 'r7',
		tooltip: conv({ hans: '“未介绍重定向名称所指事物”指导向目标完全未提及重定向名称或其别名，或虽提及但仍不含有对重定向名称所指事物的有价值描述。<br>“涵盖的主题比导向目标更广泛”指重定向名称所指事物并非导向目标本主题或其子主题，且与导向目标主题并无密不可分的联系。<br>如果重定向标题或其别名是一个列表的项目之一，而该重定向的导向目标是该列表，则不适用。<br>如果原重定向标题可改成消歧义页（或重定向至其他消歧义页），则不适用。<br>挂有{{错字重定向}}模板的页面是否适用此条应根据其对应正字判断。<br>挂有{{收录标准重定向}}或{{合并重定向}}模板的页面不适用，请改为提出存废讨论。<br>如有用户对标题用字存在未解决的争议，则应交由存废讨论处理。', hant: '「未介紹重定向名稱所指事物」指導向目標完全未提及重定向名稱或其別名，或雖提及但仍不含有對重定向名稱所指事物的有價值描述。<br>「涵蓋的主題比導向目標更廣泛」指重定向名稱所指事物並非導向目標本主題或其子主題，且與導向目標主題並無密不可分的聯繫。<br>如果重新導向標題或其別名是一個列表的專案之一，而該重新導向的導向目標是該列表，則不適用。<br>如果原重新導向標題可改成消歧義頁（或重新導向至其他消歧義頁），則不適用。<br>掛有{{错字重定向}}模板的頁面是否適用此條應根據其對應正字判斷。<br>掛有{{收錄標準重新導向}}或{{合併重新導向}}模板的頁面不適用，請改為提出存廢討論。<br>如有使用者對標題用字存在未解決的爭議，則應交由存廢討論處理。' }),
