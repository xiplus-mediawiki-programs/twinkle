...
Twinkle.unlink = function twinkleunlink() {
	...
	if( Morebits.userIsInGroup( 'sysop' ) ) {
		Twinkle.addPortletLink( Twinkle.unlink.callback, wgUVS("链入", "連入"), "tw-unlink", wgUVS("取消到本页的链接", "取消到本頁的連結") );
	}
};
...
Twinkle.unlink.callback = function(presetReason) {
	...
	Window.setTitle( wgUVS("取消链入", "取消連入") + (mw.config.get('wgNamespaceNumber') === 6 ? wgUVS("和文件使用", "和檔案使用") : "") );
	Window.setScriptName( "Twinkle" );
	Window.addFooterLink( wgUVS("Twinkle帮助", "Twinkle說明"), "WP:TW/DOC#unlink" );

	var form = new Morebits.quickForm( Twinkle.unlink.callback.evaluate );

	// prepend some basic documentation
	var node1 = Morebits.htmlNode("code", "[[" + Morebits.pageNameNorm + wgUVS("|链接文本]]", "|連結文本]]"));
	var node2 = Morebits.htmlNode("code", wgUVS("链接文本", "連結文本"));
	...
	form.append( {
		...
		label: [
			wgUVS('这个工具可以取消所有指向该页的链接（“链入”）', '這個工具可以取消所有指向該頁的連結（「連入」）') +
				(mw.config.get('wgNamespaceNumber') === 6 ? wgUVS("，和/或通过加入<!-- -->注释标记隐藏所有对此文件的使用", "，和/或通過加入<!-- -->注釋標記隱藏所有對此檔案的使用") : "") +
				"。比如，",
			node1,
			wgUVS("将会变成", "將會變成"),
			node2,
	...
	var wikipedia_api = new Morebits.wiki.api( wgUVS('抓取链入', '擷取連入'), query, Twinkle.unlink.callbacks.display.backlinks );
	...
	wikipedia_api.statelem.status( wgUVS("加载中…", "載入中…") );
...
Twinkle.unlink.callback.evaluate = function twinkleunlinkCallbackEvaluate(event) {
	...
	if (!reason) {
		alert(wgUVS("您必须指定取消链入的理由。", "您必須指定取消連入的理由。"));
		return;
	}
	...
	var unlinker = new Morebits.batchOperation(wgUVS("取消链入", "取消連入") + (imageusage ? wgUVS("与文件使用", "與檔案使用") : ""));
	unlinker.setOption("preserveIndividualStatusLines", true);
	unlinker.setPageList(pages);
	var params = { reason: reason, unlinker: unlinker };
	unlinker.run(function(pageName) {
		var wikipedia_page = new Morebits.wiki.page(pageName, wgUVS("在条目“" + pageName + "”中取消链入", "在條目「" + pageName + "」中取消連入"));
...
Twinkle.unlink.callbacks = {
	display: {
		backlinks: function twinkleunlinkCallbackDisplayBacklinks(apiobj) {
			...
			if( apiobj.params.image ) {
				...
				if (!list.length)
				{
					apiobj.params.form.append( { type: 'div', label: wgUVS('未找到文件使用。', '未找到檔案使用。') } );
				}
				else
				{
					apiobj.params.form.append( { type:'header', label: wgUVS('文件使用', '檔案使用') } );
					namespaces = [];
					...
					apiobj.params.form.append( {
						type: 'div',
						label: wgUVS("已选择的命名空间：", "已選擇的名字空間：") + namespaces.join(', '),
						tooltip: wgUVS("您可在Twinkle属性中更改这个，请参见[[WP:TWPREFS]]", "您可在Twinkle屬性中更改這個，請參見[[WP:TWPREFS]]")
					});
					if ($(xmlDoc).find('query-continue').length) {
						apiobj.params.form.append( {
							type: 'div',
							label: wgUVS("显示头 " + list.length.toString() + " 个文件使用。", "顯示頭 " + list.length.toString() + " 個檔案使用。")
						});
					}
					apiobj.params.form.append({
						type: 'button',
			...
			if( backlinks.length > 0 ) {
				...
				apiobj.params.form.append( {
					type: 'div',
					label: wgUVS("已选择的名字空间：", "已選擇的名字空間：") + namespaces.join(', '),
					tooltip: wgUVS("您可在Twinkle属性中更改这个，请参见[[WP:TWPREFS]]", "您可在Twinkle屬性中更改這個，請參見[[WP:TWPREFS]]")
				});
				if ($(xmlDoc).find('query-continue').length) {
					apiobj.params.form.append( {
						type: 'div',
						label: wgUVS("显示头 " + list.length.toString() + " 个链入。", "顯示頭 " + list.length.toString() + " 個連入。")
					});
				}
				apiobj.params.form.append({
					type: 'button',
			...
			{
				apiobj.params.form.append( { type: 'div', label: wgUVS('未找到链入。', '未找到連入。') } );
			}
	...
	unlinkBacklinks: function twinkleunlinkCallbackUnlinkBacklinks(pageobj) {
		...
		if (params.doImageusage) {
			wikiPage.commentOutImage(mw.config.get('wgTitle'), wgUVS('注释出', '注釋出'));
			text = wikiPage.getText();
			// did we actually make any changes?
			if (text === oldtext) {
				warningString = wgUVS("文件使用", "檔案使用");
			} else {
				summaryText = wgUVS("注释出文件使用", "注釋出檔案使用");
				oldtext = text;
			}
		}
		...
		if (params.doBacklinks) {
			...
			// did we actually make any changes?
			if (text === oldtext) {
				warningString = (warningString ? wgUVS("反链或文件使用", "反連或檔案使用") : wgUVS("反链", "反連"));
			} else {
				summaryText = (summaryText ? (summaryText + " / ") : "") + wgUVS("取消链接到", "取消連結到");
				oldtext = text;
			}
		}
		...
		if (warningString) {
			// nothing to do!
			pageobj.getStatusElement().error(wgUVS("未能在页面上找到", "未能在頁面上找到") + warningString + "。");
