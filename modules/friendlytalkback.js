...
Twinkle.talkback.callback = function() {
	if (mw.config.get('wgRelevantUserName') === mw.config.get('wgUserName') && !confirm(conv({ hans: '您寂寞到了要自己通告自己的程度么？', hant: '您寂寞到了要自己通告自己的程度麼？' }))) {
		return;
	}
...
Twinkle.talkback.callback.optoutStatus = function(apiobj) {
	...
	if ($el.length) {
		Twinkle.talkback.optout = mw.config.get('wgRelevantUserName') + conv({ hans: '不希望收到回复通告', hant: '不希望收到回覆通告' });
		var url = $el.text();
