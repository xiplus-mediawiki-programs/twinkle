...
Twinkle.copyvio = function twinklecopyvio() {
	...
	Twinkle.addPortletLink(Twinkle.copyvio.callback, wgUVS("侵权", "侵權"), "tw-copyvio", wgUVS("提报侵权页面", "提報侵權頁面"), "");
};

Twinkle.copyvio.callback = function twinklecopyvioCallback() {
	var Window = new Morebits.simpleWindow( 600, 350 );
	Window.setTitle( wgUVS("提报侵权页面", "提報侵權頁面") );
	Window.setScriptName( "Twinkle" );
	Window.addFooterLink( wgUVS("常见错误", "常見錯誤"), "Wikipedia:管理员错误自查表/侵权处理" );
	Window.addFooterLink( wgUVS("Twinkle帮助", "Twinkle說明"), "WP:TW/DOC#copyvio" );

	var form = new Morebits.quickForm( Twinkle.copyvio.callback.evaluate );
	form.append( {
			type: 'textarea',
			label:wgUVS('侵权来源：', '侵權來源：'),
			name: 'source'
		}
	);
	form.append( {
	...
	if (mw.config.get('wgNamespaceNumber') === 118) {
		form.append( {
				...
				list: [
					{
						label: wgUVS('同时标记CSD G16', '同時標記CSD G16'),
						value: 'csd',
						name: 'csd',
						tooltip: wgUVS('G16: 因为主页面侵权而创建的临时页面仍然侵权', 'G16: 因為主頁面侵權而建立的臨時頁面仍然侵權'),
...
Twinkle.copyvio.callbacks = {
	tryTagging: function (pageobj) {
		...
		if (text.indexOf('{{Copyvio|') === -1) {
			...
			Morebits.status.error( wgUVS('错误', '錯誤'), wgUVS('页面已经标记侵权，请人工确认是否已经提报。', '頁面已經標記侵權，請人工確認是否已經提報。') );
		}
	},
	main: function(pageobj) {
		...
		// Adding discussion
		var wikipedia_page = new Morebits.wiki.page(params.logpage, wgUVS("添加侵权记录项", "加入侵權記錄項"));
		...
		if(params.usertalk) {
			Morebits.wiki.flow.check('User talk:' + initialContrib, function () {
				var flowpage = new Morebits.wiki.flow('User talk:' + initialContrib, wgUVS("通知页面创建者（" + initialContrib + "）", "通知頁面建立者（" + initialContrib + "）"));
				...
				var usertalkpage = new Morebits.wiki.page('User talk:' + initialContrib, wgUVS("通知页面创建者（" + initialContrib + "）", "通知頁面建立者（" + initialContrib + "）"));
	...
	taggingArticle: function(pageobj) {
		...
		pageobj.setEditSummary(wgUVS("本页面疑似侵犯版权", "本頁面疑似侵犯版權") + Twinkle.getPref('summaryAd'));
	...
	copyvioList: function(pageobj) {
		...
		pageobj.setEditSummary(wgUVS("添加[[", "加入[[") + mw.config.get('wgPageName') + "]]" + Twinkle.getPref('summaryAd'));
		pageobj.setCreateOption('recreate');
		pageobj.append();
	}
};
...
Twinkle.copyvio.callback.evaluate = function(e) {
	...
	if( !source.trim() ) {
		Morebits.status.error( wgUVS('错误', '錯誤'), wgUVS('未指定侵权来源', '未指定侵權來源') );
		return;
	}
	...
	Morebits.wiki.actionCompleted.notice = wgUVS("提报完成，将在几秒内刷新", "提報完成，將在幾秒內重新整理");

	// Tagging file
	wikipedia_page = new Morebits.wiki.page(mw.config.get('wgPageName'), wgUVS("添加侵权模板到页面", "加入侵權模板到頁面"));
	wikipedia_page.setFollowRedirect(true);
	wikipedia_page.setCallbackParameters(params);
