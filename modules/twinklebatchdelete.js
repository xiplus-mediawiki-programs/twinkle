...
Twinkle.batchdelete = function twinklebatchdelete() {
	if(
		...
		Twinkle.addPortletLink( Twinkle.batchdelete.callback, wgUVS("批删", "批刪"), "tw-batch", wgUVS("删除此分类或页面中的所有链接", "刪除此分類或頁面中的所有連結") );
	}
};
...
Twinkle.batchdelete.callback = function twinklebatchdeleteCallback() {
	...
	Window.setTitle( wgUVS("批量删除", "批量刪除") );
	Window.setScriptName( "Twinkle" );
	Window.addFooterLink( wgUVS("Twinkle帮助", "Twinkle說明"), "WP:TW/DOC#batchdelete" );
			...
			list: [
				{
					...
					subgroup: {
						...
						list: [
							{
								label: wgUVS('删除关联的讨论页（用户对话页除外）', '刪除關聯的討論頁（用戶對話頁除外）'),
								name: 'delete_talk',
								value: 'delete_talk',
								checked: true
							},
				...
				{
					label: wgUVS('取消链入（仅处理条目及Portal命名空间）', '取消連入（僅處理條目及Portal名字空間）'),
					name: 'unlink_page',
					value: 'unlink',
					checked: false
				},
				{
					label: wgUVS('移除文件使用（所有命名空间）', '移除檔案使用（所有名字空間）'),
					name: 'unlink_file',
					value: 'unlink_file',
					checked: true
				}
	...
	var statelem = new Morebits.status(wgUVS("抓取页面列表", "擷取頁面列表"));
	var wikipedia_api = new Morebits.wiki.api( wgUVS('加载中…', '載入中…'), query, function( apiobj ) {
			...
			$pages.each(function(index, page) {
				...
				if (isProtected) {
					metadata.push(wgUVS("全保护，", "全保護，") +
						($editprot.attr('expiry') === 'infinity' ? wgUVS('无限期', '無限期') : (wgUVS('过期时间', '過期時間') + $editprot.attr('expiry'))));
				}
				if (ns === "6") {  // mimic what delimages used to show for files
					metadata.push(wgUVS("上传者：", "上傳者：") + $page.find('ii').attr('user'));
					metadata.push(wgUVS("最后编辑：", "最後編輯：") + $page.find('rev').attr('user'));
				} else {
					metadata.push(size + wgUVS("字节", "位元組"));
				}
				list.push({
			...
			apiobj.params.form.append({ type: 'header', label: wgUVS('待删除页面', '待刪除頁面') });
...
Twinkle.batchdelete.callback.evaluate = function twinklebatchdeleteCallbackEvaluate(event) {
	Morebits.wiki.actionCompleted.notice = wgUVS('状态', '狀態');
	Morebits.wiki.actionCompleted.postfix = wgUVS('批量删除已完成', '批量刪除已完成');

	var numProtected = $(Morebits.quickForm.getElements(event.target, 'pages')).filter(function(index, element) {
		return element.checked && element.nextElementSibling.style.color === 'red';
	}).length;
	if (numProtected > 0 && !confirm(wgUVS("您即将删除" + numProtected + "个全保护页面，确定？", "您即將刪除" + numProtected + "個全保護頁面，確定？"))) {
		return;
	}
	...
	if( ! reason ) {
		alert(wgUVS("您需要给出理由！", "您需要給出理由！"));
		return;
	}
	Morebits.simpleWindow.setButtonsEnabled( false );
	Morebits.status.init( event.target );
	if( !pages ) {
		Morebits.status.error( wgUVS('错误', '錯誤'), wgUVS('没什么要删的，取消操作', '沒什麼要刪的，取消操作') );
		return;
	}

	var pageDeleter = new Morebits.batchOperation(delete_page ? wgUVS("正在删除页面", "正在刪除頁面") : wgUVS("初始化作业请求", "初始化作業請求"));
	...
	pageDeleter.run(function(pageName) {
		...
		var wikipedia_page = new Morebits.wiki.page( pageName, wgUVS('删除页面', '刪除頁面') + pageName );
...
Twinkle.batchdelete.callbacks = {
	...
	doExtras: function( thingWithParameters ) {
		...
		if( params.unlink_page ) {
			...
			wikipedia_api = new Morebits.wiki.api( wgUVS('抓取链入', '擷取連入'), query, Twinkle.batchdelete.callbacks.unlinkBacklinksMain );
		...
		if( params.unlink_file ) {
			...
			wikipedia_api = new Morebits.wiki.api( wgUVS('抓取文件链接', '擷取檔案連結'), query, Twinkle.batchdelete.callbacks.unlinkImageInstancesMain );
			wikipedia_api.params = params;
			wikipedia_api.post();
		}
		...
		if( params.delete_page ) {
			if ( params.delete_redirects ) {
				...
				wikipedia_api = new Morebits.wiki.api( wgUVS('抓取重定向', '擷取重定向'), query, Twinkle.batchdelete.callbacks.deleteRedirectsMain );
				wikipedia_api.params = params;
				wikipedia_api.post();
			}
			if ( params.delete_talk ) {
				...
				if (pageTitle && pageTitle.namespace % 2 === 0 && pageTitle.namespace !== 2) {
					...
					wikipedia_api = new Morebits.wiki.api( wgUVS('检查讨论页是否存在', '檢查討論頁是否存在'), query, Twinkle.batchdelete.callbacks.deleteTalk );
					wikipedia_api.params = params;
	...
	deleteRedirectsMain: function( apiobj ) {
		...
		var redirectDeleter = new Morebits.batchOperation(wgUVS("删除到", "刪除到") + apiobj.params.page + "的重定向");
		redirectDeleter.setOption("chunkSize", Twinkle.getPref('batchdeleteChunks'));
		redirectDeleter.setPageList(pages);
		redirectDeleter.run(function(pageName) {
			var wikipedia_page = new Morebits.wiki.page(pageName, wgUVS("删除", "刪除") + pageName);
	...
	deleteTalk: function( apiobj ) {
		...
		var page = new Morebits.wiki.page(apiobj.params.talkPage, wgUVS("删除条目" + apiobj.params.page + "的讨论页", "刪除條目" + apiobj.params.page + "的討論頁"));
	...
	unlinkBacklinksMain: function( apiobj ) {
		...
		var unlinker = new Morebits.batchOperation(wgUVS("取消到" + apiobj.params.page + "的链入", "取消到" + apiobj.params.page + "的連入"));
		unlinker.setOption("chunkSize", Twinkle.getPref('batchdeleteChunks'));
		unlinker.setPageList(pages);
		unlinker.run(function(pageName) {
			var wikipedia_page = new Morebits.wiki.page(pageName, wgUVS("取消链入于", "取消連入於") + pageName);
	...
	unlinkImageInstancesMain: function( apiobj ) {
		...
		unlinker.setPageList(pages);
		unlinker.run(function(pageName) {
			var wikipedia_page = new Morebits.wiki.page(pageName, wgUVS("移除文件使用于", "移除檔案使用於") + pageName);
			var params = $.extend({}, apiobj.params);
			params.title = pageName;
			params.unlinker = unlinker;
			wikipedia_page.setCallbackParameters(params);
	...
	unlinkImageInstances: function( pageobj ) {
		...
		var old_text = text;
		var wikiPage = new Morebits.wikitext.page( text );
		wikiPage.commentOutImage( image , wgUVS('注释出文件，因其已被删除', '注釋出檔案，因其已被刪除') );

		text = wikiPage.getText();
		Twinkle.batchdelete.unlinkCache[ params.title ] = text;
		if( text === old_text ) {
			pageobj.getStatusElement().error( wgUVS('未能取消文件' + image + '在' + pageobj.getPageName() + '的使用', '未能取消檔案' + image + '在' + pageobj.getPageName() + '的使用') );
