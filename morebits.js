...
/**
 * Get username or ip range
 * @param {boolean} [allowBlock=false]
 * @returns {string}
 */
Morebits.relevantUserName = function (allowBlock) {
	var name = mw.config.get('wgRelevantUserName');
	if (name) {
		return name;
	} else if (['Contributions', 'IPContributions'].includes(mw.config.get('wgCanonicalSpecialPageName'))) {
		if ($('#contentSub').find('a[title^="Special:Log/block"]').length && allowBlock) {
			var link = $('#contentSub').find('a[title^="Special:Log/block"]')[0].href;
...
// Create capital letter aliases for all Morebits @classes (functions that work with the `new` keyword)
// to follow the coding convention that classes should start with an uppercase letter. This will let us start fixing ESLint `new-cap` errors in other files.
