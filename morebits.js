...
Morebits.wiki.actionCompleted.notice = wgUVS('动作', '動作');
...
Morebits.wiki.api.prototype = {
	...
	post: function( callerAjaxParameters ) {
		...
		return $.ajax( ajaxparams ).done(
			...
			function(jqXHR, statusText, errorThrown) {
				...
				this.errorText = statusText + wgUVS('在调用API时发生了错误“', '在存取API時發生了錯誤「') + jqXHR.statusText + wgUVS('”。', '」。');
				this.returnError();
			}
	...
	returnError: function() {
		if ( this.errorCode === "badtoken" ) {
			this.statelem.error( wgUVS("无效令牌，请刷新页面并重试", "無效權杖，請重新整理頁面並重試") );
		} else {
...
Morebits.wiki.page = function(pageName, currentAction) {
	...
	if (!currentAction) {
		currentAction = wgUVS('打开页面“', '開啟頁面「') + pageName + wgUVS('”', '」');
	}
	...
	this.load = function(onSuccess, onFailure) {
		...
		ctx.loadApi = new Morebits.wiki.api(wgUVS("抓取页面…", "擷取頁面…"), ctx.loadQuery, fnLoadSuccess, ctx.statusElement, ctx.onLoadFailure);
	...
	this.save = function(onSuccess, onFailure) {
		...
		if (ctx.fullyProtected && !ctx.suppressProtectWarning &&
			!confirm(wgUVS('您即将编辑全保护页面 "', '您即將編輯全保護頁面 "') + ctx.pageName +
			(ctx.fullyProtected === 'infinity' ? '（永久）' : ('（到期：' + ctx.fullyProtected + ')')) +
			wgUVS('。\n\n点击确定以确定，或点击取消以取消。', '。\n\n點選確定以繼續，或點選取消以取消。'))) {
			ctx.statusElement.error(wgUVS("已取消对全保护页面的编辑。", "已取消對全保護頁面的編輯。"));
			ctx.onSaveFailure(this);
			return;
		}
		...
		ctx.saveApi = new Morebits.wiki.api( wgUVS("保存页面…", "儲存頁面…"), query, fnSaveSuccess, ctx.statusElement, fnSaveError);
	...
	this.lookupCreator = function(onSuccess) {
		...
		ctx.lookupCreatorApi = new Morebits.wiki.api(wgUVS("抓取页面创建者信息", "擷取頁面建立者資訊"), query, fnLookupCreatorSuccess, ctx.statusElement);
	...
	this.patrol = function() {
		...
		if ( rcid ) {
			...
			var patrolstat = new Morebits.status( wgUVS('标记页面为已巡查', '標記頁面為已巡查') );

			var wikipedia_api = new Morebits.wiki.api( wgUVS('进行中…', '進行中…'), {
				action: 'patrol',
				rcid: rcid,
	...
	this.move = function(onSuccess, onFailure) {
		...
		ctx.moveApi = new Morebits.wiki.api(wgUVS("抓取移动令牌…", "擷取移動權杖…"), query, fnProcessMove, ctx.statusElement, ctx.onMoveFailure);
	...
	this.deletePage = function(onSuccess, onFailure) {
		...
		if (fnCanUseMwUserToken('delete')) {
			...
			ctx.deleteApi = new Morebits.wiki.api(wgUVS("抓取删除令牌…", "擷取刪除權杖…"), query, fnProcessDelete, ctx.statusElement, ctx.onDeleteFailure);
	...
	this.protect = function(onSuccess, onFailure) {
		...
		ctx.protectApi = new Morebits.wiki.api(wgUVS("抓取保护令牌…", "擷取保護權杖…"), query, fnProcessProtect, ctx.statusElement, ctx.onProtectFailure);
	...
	var fnLoadSuccess = function() {
		...
		if (!ctx.editToken) {
			ctx.statusElement.error(wgUVS("未能抓取编辑令牌。", "未能擷取編輯權杖。"));
			ctx.onLoadFailure(this);
			return;
		}
		ctx.loadTime = $(xml).find('page').attr('starttimestamp');
		if (!ctx.loadTime) {
			ctx.statusElement.error(wgUVS("未能抓取起始时间戳。", "未能擷取起始時間戳"));
			ctx.onLoadFailure(this);
			return;
		}
		...
		if (ctx.editMode === 'revert') {
			...
			if (!ctx.revertCurID) {
				ctx.statusElement.error(wgUVS("未能抓取当前修订版本ID。", "未能擷取目前變更版本ID。"));
				ctx.onLoadFailure(this);
				return;
			}
			...
			if (!ctx.revertUser) {
				if ($(xml).find('rev').attr('userhidden') === "") {  // username was RevDel'd or oversighted
					ctx.revertUser = wgUVS("<用户名已隐藏>", "<用戶名已隱藏>");
				} else {
					ctx.statusElement.error(wgUVS("未能抓取此修订版本的编辑者。", "未能擷取此變更版本的編輯者。"));
					ctx.onLoadFailure(this);
					return;
				}
			}
			// set revert edit summary
			ctx.editSummary = "[[WP:UNDO|取消]]由 " + ctx.revertUser + " 所做出的" + wgUVS("修订 ", "變更 ") + ctx.revertOldID + "：" + ctx.editSummary;
		}
	...
	var fnCheckPageName = function(xml, onFailure) {
		...
		if ( $(xml).find('page').attr('invalid') ) {
			ctx.statusElement.error(wgUVS("标题不合法：", "標題不合法：") + ctx.pageName);
			onFailure(this);
			return false; // abort
		}
		...
		if ( $(xml).find('page').attr('title') ) {
			...
			if ( $(xml).find('redirects').length > 0 ) {
				Morebits.status.info("Info", wgUVS("从 ", "從 ") + ctx.pageName + " 重定向到 " + resolvedName );
			}
			ctx.pageName = resolvedName;  // always update in case of normalization
		}
		else {
			// could be a circular redirect or other problem
			ctx.statusElement.error(wgUVS("不能解释页面的重定向：", "不能解釋頁面的重定向：") + ctx.pageName);
			onFailure(this);
	...
	var fnSaveSuccess = function() {
		...
		if (blacklist) {
			...
			ctx.statusElement.error(wgUVS(['不能保存页面，因URL ', code, ' 在垃圾黑名单中。'],
									['不能儲存頁面，因URL ', code, ' 在垃圾黑名單中。']));
		} else if ( $(xml).find('captcha').length > 0 ) {
			ctx.statusElement.error(wgUVS("不能保存页面，因服务器试图让您完成一个全自动区分计算机和人类的图灵测试。",
										"不能儲存頁面，因伺服器試圖讓您完成一個全自動區分電腦和人類的圖靈測試。"));
		} else if ( $editNode.attr('code') === 'abusefilter-disallowed' ) {
			ctx.statusElement.error(wgUVS('编辑被防滥用过滤器规则“' + $editNode.attr('info').substring(17) + '”阻止。',
			...
			ctx.statusElement.error(wgUVS([ '防滥用过滤器给出了如下警告：', div, '如果您仍希望做出该编辑，请重新提交。此警告不会再次出现。' ],
			...
			ctx.statusElement.error(wgUVS("保存页面时由API得到未知错误", "儲存頁面時由API得到未知錯誤"));
		}
	...
	var fnSaveError = function() {
		...
		if ( errorCode === "editconflict" && ctx.conflictRetries++ < ctx.maxConflictRetries ) {
			...
			var purgeApi = new Morebits.wiki.api(wgUVS("检测到编辑冲突，更新服务器缓存", "檢測到編輯衝突，更新伺服器快取"),
			...
			ctx.statusElement.info(wgUVS("检测到编辑冲突，重试修改", "檢測到編輯衝突，重試修改"));
			if (fnCanUseMwUserToken('edit')) {
		...
		// it's impractical to request a new token here, so invoke edit conflict logic when this happens
			...
			ctx.statusElement.info(wgUVS("编辑令牌不可用，重试", "編輯權杖不可用，重試"));
			--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
			if (fnCanUseMwUserToken('edit')) {
		...
		// check for network or server error
			...
			ctx.statusElement.info(wgUVS("保存失败，重试", "儲存失敗，重試"));
			--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
		...
		// hard error, give up
			...
			if ( errorCode === "protectedpage" ) {
				ctx.statusElement.error( wgUVS("不能保存修改：页面被全保护", "不能儲存修改：頁面被全保護") );
			} else if ( errorCode === "hookaborted" ) {
				ctx.statusElement.error( wgUVS("不能保存修改：被防滥用过滤器阻止", "不能儲存修改：被防濫用過濾器阻止") );
			} else {
				ctx.statusElement.error( wgUVS("不能保存修改：", "不能儲存修改：") + ctx.saveApi.getErrorText() );
			}
			ctx.editMode = 'all';  // cancel append/prepend/revert modes
	...
	var fnLookupCreatorSuccess = function() {
		...
		if (!ctx.creator) {
			ctx.statusElement.error(wgUVS("不能获取页面创建者的名字", "無法取得頁面建立者的名字"));
			return;
		}
	...
	var fnProcessMove = function() {
		...
		if ($(xml).find('page').attr('missing') === "") {
			ctx.statusElement.error(wgUVS("不能移动页面，因其已不存在", "無法移動頁面，因其已不存在"));
			ctx.onMoveFailure(this);
			return;
		}
		...
		if (Morebits.userIsInGroup('sysop')) {
			...
			if (editprot.length > 0 && editprot.attr('level') === 'sysop' && !ctx.suppressProtectWarning &&
				!confirm(wgUVS('您即将移动全保护页面“', '您即將移動全保護頁面「') + ctx.pageName + wgUVS('”', '」') +
				(editprot.attr('expiry') === 'infinity' ? '（永久）' : ('（到期：' + editprot.attr('expiry') + '）')) +
				wgUVS('。\n\n点击确定以确定，或点击取消以取消。', '。\n\n點選確定以繼續，或點選取消以取消。'))) {
				ctx.statusElement.error(wgUVS("对全保护页面的移动已取消。", "對全保護頁面的移動已取消。"));
				ctx.onMoveFailure(this);
				return;
			}
		}
		...
		if (!moveToken) {
			ctx.statusElement.error(wgUVS("不能抓取移动令牌。", "不能擷取移動權杖。"));
			ctx.onMoveFailure(this);
			return;
		}
		...
		ctx.moveProcessApi = new Morebits.wiki.api(wgUVS("移动页面…", "移動頁面…"), query, ctx.onMoveSuccess, ctx.statusElement, ctx.onMoveFailure);
	...
	var fnProcessDelete = function() {
		...
		if (fnCanUseMwUserToken('delete')) {
			...
			if ($(xml).find('page').attr('missing') === "") {
				ctx.statusElement.error(wgUVS("不能删除页面，因其已不存在", "不能刪除頁面，因其已不存在"));
				ctx.onDeleteFailure(this);
				return;
			}
			...
			var editprot = $(xml).find('pr[type="edit"]');
			if (editprot.length > 0 && editprot.attr('level') === 'sysop' && !ctx.suppressProtectWarning &&
				!confirm(wgUVS('您即将删除全保护页面“' + ctx.pageName + "”", '您即將刪除全保護頁面「' + ctx.pageName + "」") +
				(editprot.attr('expiry') === 'infinity' ? '（永久）' : ('（到期 ' + editprot.attr('expiry') + '）')) +
				wgUVS('。\n\n点击确定以确定，或点击取消以取消。', '。\n\n點選確定以確定，或點選取消以取消。'))) {
				ctx.statusElement.error(wgUVS("对全保护页面的删除已取消。", "對全保護頁面的刪除已取消。"));
				ctx.onDeleteFailure(this);
				return;
			}
			...
			if (!token) {
				ctx.statusElement.error(wgUVS("不能抓取删除令牌。", "不能擷取刪除權杖。"));
				ctx.onDeleteFailure(this);
				return;
			}
		...
		ctx.deleteProcessApi = new Morebits.wiki.api(wgUVS("删除页面…", "刪除頁面…"), query, ctx.onDeleteSuccess, ctx.statusElement, fnProcessDeleteError);
	...
	var fnProcessDeleteError = function() {
		...
		if ( errorCode === "internal_api_error_DBQueryError" && ctx.retries++ < ctx.maxRetries ) {
			...
			ctx.statusElement.info(wgUVS("数据库查询错误，重试", "資料庫查詢錯誤，重試"));
			--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
			...
			ctx.statusElement.error(wgUVS("无效令牌，请刷新页面并重试。", "無效權杖，請重新整理頁面並重試。"));
		...
		// hard error, give up
			...
			ctx.statusElement.error( wgUVS("不能删除页面：", "不能刪除頁面：") + ctx.deleteProcessApi.getErrorText() );
			if (ctx.onDeleteFailure) {
				ctx.onDeleteFailure.call(this, ctx.deleteProcessApi);  // invoke callback
			}
		}
	...
	var fnProcessProtect = function() {
		...
		if (((ctx.protectEdit || ctx.protectMove) && missing)) {
			ctx.statusElement.error(wgUVS("不能保护页面，因其已不存在", "不能保護頁面，因其已不存在"));
			ctx.onProtectFailure(this);
			return;
		}
		if (ctx.protectCreate && !missing) {
			ctx.statusElement.error(wgUVS("不能白纸保护页面，因其已存在", "不能白紙保護頁面，因其已存在"));
			ctx.onProtectFailure(this);
			return;
		}
		...
		if (!protectToken) {
			ctx.statusElement.error(wgUVS("不能抓取保护令牌。", "不能擷取保護權杖。"));
			ctx.onProtectFailure(this);
			return;
		}
		...
		ctx.protectProcessApi = new Morebits.wiki.api(wgUVS("保护页面…", "保護頁面…"), query, ctx.onProtectSuccess, ctx.statusElement, ctx.onProtectFailure);
...
//        watchlistOption: 'nochange',
    ...
    this.viewHeader = function (onSuccess, onFailure) {
        ...
        ctx.viewHeaderApi = new Morebits.wiki.api(wgUVS('抓取Flow描述…', '擷取Flow描述…'), query, fnViewHeaderSuccess, ctx.statusElement, ctx.onViewHeaderFailure);
    ...
    this.editHeader = function (onSuccess, onFailure) {
        ...
        ctx.editHeaderApi = new Morebits.wiki.api( wgUVS("编辑Flow讨论页描述…", "編輯Flow討論頁描述…"), query, fnEditHeaderSuccess, ctx.statusElement, fnEditHeaderError);
    ...
    var fnNewTopicError = function() {
        ...
        if (errorCode === 'invalid-page') {
            ctx.statusElement.error(wgUVS("内部错误：不是Flow页面，无法留言", "內部錯誤：不是Flow頁面，無法留言"));
        } else if (errorCode === 'block') {
            ctx.statusElement.error(wgUVS("无法留言，因讨论页被保护", "無法留言，因討論頁被保護"));
        } else if (errorCode === 'spamfilter') {
            ctx.statusElement.error(wgUVS("无法留言，因为需要验证码或已经触发URL黑名单", "無法留言，因為需要驗證碼或已經觸發URL黑名單"));
        } else {
            ctx.statusElement.error(wgUVS("留言时由API得到未知错误", "留言時由API得到未知錯誤"));
        }
    ...
    var fnEditHeaderSuccess = function() {
        ...
        if ($(xml).find('edit-header').attr('status') === "ok") {
            ...
            ctx.statusElement.error(wgUVS("保存Flow讨论页描述时由API得到未知错误", "儲存Flow討論頁描述時由API得到未知錯誤"));
    ...
    var fnEditHeaderError = function() {
        ...
        if (errorCode === 'invalid-page') {
            ctx.statusElement.error(wgUVS("内部错误：不是Flow页面，无法编辑描述", "內部錯誤：不是Flow頁面，無法編輯描述"));
        } else if (errorCode === 'block') {
            ctx.statusElement.error(wgUVS("无法编辑描述，因讨论页被保护", "無法編輯描述，因討論頁被保護"));
        } else if (errorCode === 'spamfilter') {
            ctx.statusElement.error(wgUVS("无法编辑描述，因为需要验证码或已经触发URL黑名单", "無法編輯描述，因為需要驗證碼或已經觸發URL黑名單"));
        } else {
            ctx.statusElement.error(wgUVS("编辑描述时由API得到未知错误", "編輯描述時由API得到未知錯誤"));
        }
...
Morebits.wiki.flow.check = function(title, callbackOnFlow, callbackOnNonFlow, onError) {
    ...
    var statusElement = new Morebits.status(wgUVS('检查是否为Flow页面', '檢查是否為Flow頁面'));
    var checkApi = new Morebits.wiki.api(wgUVS("查询页面信息", "檢索頁面信息"), {
        action: 'query',
        prop: 'info',
        titles: title,
...
Morebits.wiki.preview = function(previewbox) {
	...
	this.beginRender = function(wikitext, pageTitle) {
		...
		var renderApi = new Morebits.wiki.api(wgUVS("加载中…", "載入中…"), query, fnRenderSuccess, new Morebits.status(wgUVS("预览", "預覽")));
		renderApi.post();
	};
	...
	var fnRenderSuccess = function(apiobj) {
		...
		if (!html) {
			apiobj.statelem.error(wgUVS("加载预览失败，或模板被清空", "載入預覽失敗，或模板被清空"));
			return;
		}
...
Morebits.batchOperation = function(currentAction) {
	var ctx = {
		...
		statusElement: new Morebits.status(currentAction || wgUVS("执行批量操作", "執行批量操作" )),
		worker: null,
		countStarted: 0,
		countFinished: 0,
	...
	this.run = function(worker) {
		if (ctx.running) {
			ctx.statusElement.error(wgUVS("批量操作已在运行", "批量操作已在執行"));
			return;
		}
		ctx.running = true;
		...
		if (!total) {
			ctx.statusElement.info(wgUVS("没什么要做的", "沒什麼要做的"));
			ctx.running = false;
			return;
		}
	...
	var fnDoneOne = function() {
		...
		if (ctx.countFinished > total) {
			ctx.statusElement.warn(wgUVS("完成（多执行了" + (ctx.countFinished - total) + "）", "完成（多執行了" + (ctx.countFinished - total) + "）"));
			Morebits.wiki.removeCheckpoint();
			ctx.running = false;
			return;
		}
